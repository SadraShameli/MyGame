...
namespace MyGame
{
	...
	public:
		...
		void Create(const std::wstring_view& DebugHeapName, D3D12_DESCRIPTOR_HEAP_TYPE Type, uint32_t MaxCount);
		...
		uint32_t GetOffsetOfHandle(const DescriptorHandle& DHandle) { return (uint32_t)(DHandle.GetCpuPtr() - m_FirstHandle.GetCpuPtr()) / m_DescriptorSize; }
