...
namespace MyGame
{
	...
	GraphicsPSO::GraphicsPSO(const wchar_t* Name) : PipelineState(Name)
	{
	...
	{
		MYGAME_ASSERT(TopologyType != D3D12_PRIMITIVE_TOPOLOGY_TYPE_UNDEFINED, "Can't draw with undefined topology");
		m_PSODesc.PrimitiveTopologyType = TopologyType;
	}

	void GraphicsPSO::SetPrimitiveRestart(D3D12_INDEX_BUFFER_STRIP_CUT_VALUE IBProps) { m_PSODesc.IBStripCutValue = IBProps; }
	...
	void GraphicsPSO::SetRenderTargetFormat(DXGI_FORMAT RTVFormat, DXGI_FORMAT DSVFormat, UINT MsaaCount, UINT MsaaQuality) { SetRenderTargetFormats(1, &RTVFormat, DSVFormat, MsaaCount, MsaaQuality); }
	...
	{
		MYGAME_ASSERT(NumRTVs == 0 || RTVFormats != nullptr, "Null format array conflicts with non-zero length");
		for (UINT i = 0; i < NumRTVs; ++i)
		{
			MYGAME_ASSERT(RTVFormats[i] != DXGI_FORMAT_UNKNOWN);
	...
	void GraphicsPSO::Finalize()
	{
		...
		if (firstCompile)
		{
			...
			ThrowIfFailed(DirectXImpl::m_device->CreateGraphicsPipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO)));
	...
	void ComputePipelineState::Finalize()
	{
		...
		if (firstCompile)
		{
			ThrowIfFailed(DirectXImpl::m_device->CreateComputePipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO)));
