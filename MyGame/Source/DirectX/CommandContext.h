...
#define VALID_COMPUTE_QUEUE_RESOURCE_STATES \
	...
	public:
		...
		uint64_t Flush(bool = false);
		uint64_t Finish(bool = false);

		GraphicsContext& GetGraphicsContext()
		{
			MYGAME_ASSERT(m_Type != D3D12_COMMAND_LIST_TYPE_COMPUTE, "Cannot convert async compute context to graphics");
		...
		void ResetCounter(StructuredBuffer& Buf, uint32_t);
	...
	inline void ComputeContext::SetConstantArray(UINT RootEntry, UINT NumConstants, const void* pConstants) { m_CommandList->SetComputeRoot32BitConstants(RootEntry, NumConstants, pConstants, 0); }
	inline void ComputeContext::SetConstant(UINT RootEntry, UINT Offset, DWParam Val) { m_CommandList->SetComputeRoot32BitConstant(RootEntry, Val.Uint, Offset); }
	inline void ComputeContext::SetConstants(UINT RootEntry, DWParam X) { m_CommandList->SetComputeRoot32BitConstant(RootEntry, X.Uint, 0); }
	...
	inline void ComputeContext::SetConstantBuffer(UINT RootIndex, D3D12_GPU_VIRTUAL_ADDRESS CBV) { m_CommandList->SetComputeRootConstantBufferView(RootIndex, CBV); }
	inline void GraphicsContext::SetConstantBuffer(UINT RootIndex, D3D12_GPU_VIRTUAL_ADDRESS CBV) { m_CommandList->SetGraphicsRootConstantBufferView(RootIndex, CBV); }
	...
	{
		MYGAME_ASSERT(IndexData != nullptr && Math::IsAligned(IndexData, 16));
	...
	inline void GraphicsContext::SetDynamicSRV(UINT RootIndex, size_t BufferSize, const void* BufferData)
	{
		MYGAME_ASSERT(BufferData != nullptr && Math::IsAligned(BufferData, 16));
		DynAlloc cb = m_CpuLinearAllocator.Allocate(BufferSize);
		memcpy(cb.DataPtr, BufferData, Math::AlignUp(BufferSize, 16) >> 4);
	...
	{
		MYGAME_ASSERT((SRV.m_UsageState & (D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)) != 0);
	...
	{
		MYGAME_ASSERT((SRV.m_UsageState & D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE) != 0);
	...
	{
		MYGAME_ASSERT((UAV.m_UsageState & D3D12_RESOURCE_STATE_UNORDERED_ACCESS) != 0);
