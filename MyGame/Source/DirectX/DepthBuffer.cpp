...
namespace MyGame
{
	...
	void DepthBuffer::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format)
	{
		...
		{
			m_hDSV[0] = DirectXImpl::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
			m_hDSV[1] = DirectXImpl::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
		}
		...
		if (stencilReadFormat != DXGI_FORMAT_UNKNOWN)
		{
			...
			{
				m_hDSV[2] = DirectXImpl::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
				m_hDSV[3] = DirectXImpl::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
			}
		...
		if (m_hDepthSRV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
			m_hDepthSRV = DirectXImpl::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
		...
		if (stencilReadFormat != DXGI_FORMAT_UNKNOWN)
		{
			if (m_hStencilSRV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
				m_hStencilSRV = DirectXImpl::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
